...
template <class TYPE, class DTYPE> class taskScheduler {
 ...
 */
template <class TYPE, class DTYPE> taskScheduler<TYPE, DTYPE>::taskScheduler(int clockSpeed) {
  clk = clockSpeed;
  targetDT = td_milliseconds(1000.0 / clk);
}

/**
 * Compares the execution time of a command to the expected delta time, if the time is less the thread waits until dt is
 * reached before continuing execution
 *
 * @param executionTime
 */
template <class TYPE, class DTYPE> void taskScheduler<TYPE, DTYPE>::waitOnDT(std::chrono::milliseconds executionTime) {
 ...
 */
template <class TYPE, class DTYPE> double taskScheduler<TYPE, DTYPE>::executeCommand(TYPE func, DTYPE data) {
